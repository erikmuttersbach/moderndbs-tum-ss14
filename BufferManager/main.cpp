...
static void* readWrite(void *arg) {
    ...
    pthread_setname_np(("rw-"+to_string(threadNum)).c_str());
...
int main() {
    ...
    for(unsigned int i=0; i<threadCount; i++) {
        BufferFrame& frame = mgr.fixPage(PAGEID(1, i), true);
        (static_cast<unsigned int*>(frame.getData()))[0] = i+1;
        mgr.unfixPage(frame, true);
    }
